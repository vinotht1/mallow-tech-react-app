...
export const createUserApi = async (
  ...
  try {
    const response = await APIREQUEST.post(endpoints.createUser, user);
    if (response?.data) {
...
export const updateUserApi = async (
  ...
  try {
    const response = await APIREQUEST.put(`${endpoints.updateUser}/${id}`, user);
    if (response?.data) {
      return response.data as User;
    } else {
      return { error: true, message: "No data received from server" };
...
export const deleteUserApi = async (
  id: number
): Promise<{ success: boolean } | SigninError> => {
  try {
    const response = await APIREQUEST.delete(`${endpoints.deleteUser}/${id}`);
